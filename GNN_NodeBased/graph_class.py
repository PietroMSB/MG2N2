...
class GraphObject:
	...
	def __init__(self, arcs, nodes, targets, ArcNode=None, node_aggregation = "average"):
		...
		#copy dimensions
		self.DIM_NODE_LABEL = nodes.shape[1]-2 #first column contains node indices, last column contains the output mask
	...
	def buildSetMask(self):
		self.mask_set = np.ones(len(self.nodes), dtype=bool)

	#build output mask
	def buildOutputMask(self):
		self.output_mask = np.zeros(len(self.nodes), dtype=bool)
		for i in range(len(self.nodes)):
			if self.nodes[i][-1] == 1: #output nodes are identified by a 1 in the last column of the <nodes> matrix
