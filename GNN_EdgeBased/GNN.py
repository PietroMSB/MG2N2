...
class GNN:
	...
	def Loop(self):
		...
		with tf.variable_scope(self.namespace+'loop'):
			...
			#gather source node states
			source_state = tf.cast(tf.gather(st, tf.cast(self.a[:, 0], tf.int32)), tf.float32)
			#gather destination node states
			destination_state = tf.cast(tf.gather(st, tf.cast(self.a[:, 1], tf.int32)), tf.float32)
			#crop arc labels
			arc_label = tf.slice(self.a, [0, 2], [tf.shape(self.a)[0], tf.shape(self.a)[1]-2])
			#concatenate source and destination states to arc labels
			arc_state = tf.concat([source_state, destination_state, arc_label], axis=1)
			#gather the state of arcs in the set by applying <set_mask> to <arc_state>
			set_st = tf.boolean_mask(arc_state, self.Mask, name="boolean_mask_set")
			#gather the state of output arcs by applying <output_mask> to <set_st>
